const API_ROUTES = {
  ...
  admin: {
    logs: (queryString) => `/api/v1/admin/logs?${queryString}`,
    syncSchedule: () => "/api/v1/admin/sync/schedule",
    syncStatus: () => "/api/v1/admin/sync/status",
    usersSummary: () => "/api/v1/admin/users/summary",
    users: () => "/api/v1/admin/users",
    user: (userId) => `/api/v1/admin/users/${userId}`,
    userInvite: () => "/api/v1/admin/users/invite",
    sessions: () => "/api/v1/admin/sessions",
    session: (sessionId) => `/api/v1/admin/sessions/${sessionId}`,
    securityEvents: (qs) => qs ? `/api/v1/admin/security/events?${qs}` : "/api/v1/admin/security/events",
    databaseSummary: () => "/api/v1/admin/database/summary",
    config: (category) => category ? `/api/v1/admin/config?category=${category}` : "/api/v1/admin/config",
    configItem: (key) => `/api/v1/admin/config/${encodeURIComponent(key)}`,
    grids: () => "/api/v1/admin/grids",
    gridPages: () => "/api/v1/admin/grids/pages",
    gridColumns: (gridId) => `/api/v1/admin/grids/${gridId}/columns`,
    gridColumn: (gridId, columnId) => `/api/v1/admin/grids/${gridId}/columns/${columnId}`,
    grid: (gridId) => `/api/v1/admin/grids/${gridId}`,
  ...
  userPreferences: {
    ...
    grid: (gridId) => `/api/v1/user-preferences/grids/${gridId}`,
    gridColumn: (gridId, columnId) => `/api/v1/user-preferences/grids/${gridId}/columns/${columnId}`
  },
  // ── App-wide config ──────────────────────────────────────────────────────
  appConfig: {
  ...
  diagnostics: {
    ...
    run: (runId) => `/api/v1/diagnostics/runs/${runId}`,
  ...
  auth: {
    ...
    googleAuthorize: (state) => state ? `/api/v1/auth/google/authorize?state=${encodeURIComponent(state)}` : "/api/v1/auth/google/authorize",
