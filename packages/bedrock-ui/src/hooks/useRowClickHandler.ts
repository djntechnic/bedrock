 ...
 */
export function useRowClickHandler<T extends FlyoutRowData>(
  config: GridConfig,
  visibleRows: Row<T>[],
): (row: Row<T>, index: number) => void {
  const { openFlyout } = useFlyoutStore();

  return function handleRowClick(row: Row<T>, index: number) {
    const { player_id } = row.original;
    if (!player_id) return;

    const flatRows = visibleRows.map((r) => ({
      player_id: r.original.player_id as number,
