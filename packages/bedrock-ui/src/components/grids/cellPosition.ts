 ...
 */
export function cellPositionClasses(
  pinnedSide: PinnedSide,
  isNameCol: boolean,
  isCellSelectable: boolean,
): string {
  if (pinnedSide === "left" || pinnedSide === "right") {
    // `sticky` is itself a positioned ancestor, so the fill handle still
    // anchors correctly without `relative`. The offset comes from the inline
    // `left`/`right` style the caller computes from `getStart`/`getAfter`.
