...
/** Resolver used when the host app registered nothing: never tints. */
const NO_ACCENT: RowAccentResolver = () => undefined;

/** Hook shape for the unregistered case. Calls no hooks of its own. */
const useNoAccent: RowAccentHook = () => NO_ACCENT;

let registeredHook: RowAccentHook | null = null;
 ...
 */
export function registerRowAccentResolver(hook: RowAccentHook): void {
  registeredHook = hook;
}
 ...
 */
export function useRowAccentResolver(enabled: boolean): RowAccentResolver {
...
export function __clearRowAccentResolver(): void {
  registeredHook = null;
}
