...
interface Registered extends SearchSource {
  seq: number;
}

const sources = new Map<string, Registered>();
let seq = 0;
let allTarget: SearchAllTarget | null = null;
...
export function registerSearchAllTarget(target: SearchAllTarget): void {
  allTarget = target;
}

/** :returns: The registered "see all" destination, or null. */
export function getSearchAllTarget(): SearchAllTarget | null {
  return allTarget;
}

/** Test helper: drops every registration. Not used by application code. */
export function __clearSearchSources(): void {
  sources.clear();
  allTarget = null;
}
