...
### Task 6.1: Prefix S013 and S014 Audit Scripts and Reconcile Ecosystem Tooling Dependencies

**Files:**

- Rename: `packages/bedrock-api/bedrock/tools/audit_api_docs.py` -> `packages/bedrock-api/bedrock/tools/audit_s013_api_docs.py`
- Rename: `packages/bedrock-api/bedrock/tools/audit_ledger_freshness.py` -> `packages/bedrock-api/bedrock/tools/audit_s014_ledger_freshness.py`
- Modify: `docs/standards/s013-api-standards.md`
- Modify: `docs/standards/s014-issue-logging-and-ecosystem-governance.md`
...
- Modify: `bedrock.toml`
- Modify: `packages/bedrock-ui/src/components/admin/ApiRoutesPanel.tsx`
- Modify: `packages/bedrock-api/tests/test_audit_api_docs.py`
- Modify: `packages/bedrock-api/tests/test_audit_ledger_freshness.py`
- Modify: `docs/specs/2026-09-17-api-standards-sdlc-and-issue-governance-design.md`

- [x] **Step 1: Rename audit tools with canonical prefix**
      Rename `audit_api_docs.py` -> `audit_s013_api_docs.py` and `audit_ledger_freshness.py` -> `audit_s014_ledger_freshness.py` for 1:1 standards parity.

- [x] **Step 2: Update configuration and suite orchestration**
      Configure `[tool.bedrock.audit.s013]` and `[tool.bedrock.audit.s014]` in `bedrock.toml` and `_config.py`. Add `ApiRoutesPanel.tsx` to `presentational_tables` and fix semantic tokens. Wire `audit_s013_api_docs` and `audit_s014_ledger_freshness` into `run_all.py` `AUDIT_MODULES` (S001-S014).

- [x] **Step 3: Update unit tests and documentation**
      Update imports in `test_audit_api_docs.py` and `test_audit_ledger_freshness.py`. Harmonize docs in `s013`, `s014`, `README.md`, and design spec.

- [x] **Step 4: Verify test suite and audit runner**
      Run `pytest packages/bedrock-api/tests -k audit` and `python -m bedrock.tools.run_all --root .`.

- [x] **Step 5: Commit**
      Run: `git commit -m "chore(tools): rename audit tools to audit_s013_api_docs and audit_s014_ledger_freshness for 1:1 standards parity"`
