...
2. - [x] **Unified `.gitignore` Baseline, Cross-Repo Sandbox Permissions & PreToolUse Hook Guard**: Deployed the canonical 7-section master `.gitignore` (`.antigravityrc.local`, `.claude/state/`, `scratch/`, `.testmondata*`) across `claude-kit`, `bedrock`, `CollectIt`, and `MLBTracker`, preserving `!packages/bedrock-ui/dist/` (bedrock) and `!data/rankings.json` (CollectIt, MLBTracker). Deployed `.antigravityrc.local` and `.claude/settings.local.json` with the shared `additionalDirectories`/`allow` sandbox permissions block in all 4 repos. Authored `claude-kit/hooks/pre-tool-guard.ps1` (mirrored into each repo's `.claude/hooks/`) enforcing kebab-case under `docs/` and blocking deprecated-folder writes (exit 2), registered via `PreToolUse` in each repo's `.claude/settings.json`. `git status --porcelain` confirmed clean/expected ignore behavior across all 4 repos.
3. - [x] **Centralize Agentic Tooling Architecture, Domain Doctrine, Declarative Manifests & Multi-Repo Junction Engine**: Flattened `claude-kit`'s taxonomy into `rules/`, `skills/`, `hooks/`, and `agents/` (9 canonical dual-target agent JSONs compiled to both `.claude/agents/*.md` and `.agents/agents/*.md`); deployed per-repo `agentic.toml` manifests to `bedrock`, `CollectIt`, and `MLBTracker`; implemented `C:\Dev\claude-kit\scripts\Sync-AgenticTooling.ps1` (manifest-driven junction/hardlink mount, stale-link replacement, `$HOME\.gemini\skills` / `$HOME\.claude\skills` fallback discovery) and ran it with `-All`, confirmed idempotent on a second run. Authored the runbook at `claude-kit/docs/guide/authoring-and-deploying-agentic-tooling.md`. Registered Windows Scheduled Task `Bedrock-Sync-AgenticTooling` (daily at 03:00); the at-logon trigger requires admin elevation not available in this session and is still outstanding. Injected `sync-agentic-tooling` / `Invoke-AgenticTool` aliases into `$PROFILE`.
