...
5. **Tiered Verification Protocol:**
   - _During iteration:_ Fast, lean delta tests only (`vitest related`, `pytest --testmon -q`).
   - _Before PR creation/merge:_ Comprehensive scoped test suites (`pytest -m "not integration"`, `npm run test:run`, `npx tsc -b --noEmit`) and domain/platform audit gates.
...
3. **Mode C: Feature / Task:** Enforces the platform boundary test (_A backend file belongs to the application iff it touches an application table_), identifies extension point types, and drafts `.github/ISSUE_TEMPLATE/feature_task.md`.
