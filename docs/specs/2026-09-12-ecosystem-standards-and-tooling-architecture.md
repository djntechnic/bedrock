...
Each consumer repository maintains an identical PowerShell runner `scripts/run_audit.ps1`:

```powershell
<#
.SYNOPSIS
    Unified Quality Gate and Audit Runner for CollectIt / MLBTracker.
.DESCRIPTION
    Dispatches Bedrock Platform Audits (S001-S012) and App-Specific Domain Audits (S100+).
.EXAMPLE
    .\scripts\run_audit.ps1 -All
    .\scripts\run_audit.ps1 -Standard S002
...
if ($Standard) {
    $stdNum = $Standard.ToUpper().Replace("S", "")
    $module = "bedrock.tools.audit_s$stdNum"
    & $VenvPython -m $module
    exit $LASTEXITCODE
}
...
if ($Domain) {
    ...
    Get-ChildItem -Path (Join-Path $PSScriptRoot "maintenance") -Filter "audit_s1*.py" | ForEach-Object {
    Write-Host "Running Domain Audits from scripts/audits/..." -ForegroundColor Cyan
    Get-ChildItem -Path (Join-Path $PSScriptRoot "audits") -Filter "audit_s1*.py" | ForEach-Object {
        & $VenvPython $_.FullName
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    exit 0
}

# Default / -All: Execute bedrock run_all plus domain audits
& $VenvPython -m bedrock.tools.run_all
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
